...
#         return repl_node
    ...
    def height(self):
        ...
        # your code here
        max_h = self._height_aux(self._root)
    ...
    def _height_aux(self,node):
        ...
        if node is None:
            ...
            max_h = (self._height_aux(node._left)+1) and (self._height_aux(node._right)) + 1
    ...
    def _is_identical_aux(self,node,other_node):
        ...
        if node == None and other_node == None:
            identical = True
        
        elif node is None and other_node is not None:
            identical = False
            
        elif node is not None and other_node is None:
            identical = False
   
        elif node is not None and other_node is not None:
            
            if node._value == other_node._value and\
            self._is_identical_aux(node._left,other_node._left) and \
            self._is_identical_aux(node._right,other_node._right):
                identical = True
    ...
    def parent(self, key):
        ...
        parent = None
        found = False
        while node is not None and found == False:
            if key< node._value:
                parent = node
                node = node._left
        ...
        if parent is None or found == False:
            value = None
    ...
    def parent_r(self, key):
        ...
        # your code here
        parent,found = self._parent_r_aux(key, self._root, self._root)
        if parent is None or found is False:
            value = None
        ...
        if node is not None:
            ...
            if key in self:
                ...
                if key == node._value:
                    ...
                    temp = node
                    parent, found = self._parent_r_aux(key, node._left,node)
                    
                elif key > node._value:
                    temp = node
                    parent, found = self._parent_r_aux(key, node._right,node)
    ...
    def one_child_count(self):
        ...
        if self._root is None:
            ...
            while (len(q)>0):
                ...
                if node._left != None and node._right == None or \
                node._left == None and node._right != None:
    ...
    def _is_balanced_aux(self,node):
        ...
        if node is not None:
            ...
            left = self._height_aux(node._left)
            right = self._height_aux(node._right)
            if (abs(left-right)<=1) and self._is_balanced_aux(node._left) is True and\
            self._is_balanced_aux(node._right) is True:
                balanced = True
    ...
    def _is_valid_aux(self,node):
        ...
        elif node._left != None and node._left._value > node._value:
            valid = False
            
        elif node._right != None and node._right._value < node._value:
            valid = False
    ...
    def _postorder_aux(self,node):
        ...
        b = []
        a = []
        while (node and node._value not in b):  
              
            # Visited left subtree  
            if (node._left and node._left._value not in b): 
                node = node._left  
            # Visited right subtree  
            elif (node._right and node._right._value not in b): 
                node = node._right  
              
            # Print node  
            else: 
                a.append(node._value)
                b.append(node._value)  
                node = self._root
